# Script Name
Teams generator

## Description
This script is a random teams generator: you input a number of players and the program put this inputed players in 2 different teams,up to 20 inputed players

## Output
The script produces 2 different teams with different inputed players

## Libraries
To run this script, you need the following Python libraries:
- 'random'
- 'time'

## Main Script Features
- More options to input more or less number of players
- Equal number of players on every team

## Quick Updates Hystory
- Total Updates: 1
- Total Updates Of Fixed Bugs: 1
- Total Updates Of Adding New Features: 0
- Total Updates Of Other Types Of Updates: 0

## How To Use The Script?
- Put the script in your IDE
- Be sure you downloaded the Python Libraries
- If you dont know how to download Python Libraries, click Win + R, enter "cmd" in input field and write "pip install {library's name what do you want to use}"
- After this steps from above me, you can run the code without errors